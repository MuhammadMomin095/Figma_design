'use client';

import React from 'react';
import Navbar from '../components/Navbar';
import Link from 'next/link';



const DetailPage = () => {
    return (
        <div>
          {/* Navbar */}
          <Navbar />
    
          {/* Detail Section */}



          <div className="w-[1170px] h-[1291px]">
  <div className="flex justify-start items-center absolute left-[135px] top-[222px] gap-3">
    <Link href="/Account">
    <p className="flex-grow-0 flex-shrink-0 opacity-50 text-sm text-left text-black">Account</p>
    </Link>
    <svg
      width="7"
      height="13"
      viewBox="0 0 7 13"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className="flex-grow-0 flex-shrink-0 opacity-50"
      preserveAspectRatio="none"
    >
      <line
        opacity="0.5"
        x1="6.4453"
        y1="0.852389"
        x2="0.445304"
        y2="12.6024"
        stroke="black"
      ></line>
    </svg>
    <p className="flex-grow-0 flex-shrink-0 opacity-50 text-sm text-left text-black">Gaming</p>
    <svg
      width="7"
      height="13"
      viewBox="0 0 7 13"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className="flex-grow-0 flex-shrink-0 opacity-50"
      preserveAspectRatio="none"
    >
      <line
        opacity="0.5"
        x1="6.4453"
        y1="0.852389"
        x2="0.445304"
        y2="12.6024"
        stroke="black"
      ></line>
    </svg>
    <p className="flex-grow-0 flex-shrink-0 text-sm text-left text-black">Havic HV G-92 Gamepad</p>
  </div>
  <div
    className="w-[500px] h-[600px] absolute left-[335px] top-[323px] overflow-hidden rounded bg-neutral-100"
  >
    <img
      src="images/zz.png"
      className="w-[446px] h-[315px] absolute left-[26px] top-[153px] object-none"
    />
  </div>
  <div
    className="w-[170px] h-[138px] absolute left-[135px] top-[323px] overflow-hidden rounded bg-neutral-100"
  >
    <img src="images/v.png" className="w-[121px] h-[114px] absolute left-[30px] top-[11px] object-none" />
  </div>
  <div
    className="w-[170px] h-[138px] absolute left-[135px] top-[477px] overflow-hidden rounded bg-neutral-100"
  >
    <img src="images/w.png" className="w-28 h-[97px] absolute left-[40px] top-[19px] object-none" />
  </div>
  <div
    className="w-[170px] h-[138px] absolute left-[135px] top-[631px] overflow-hidden rounded bg-neutral-100"
  >
    <img
      src="images/y.png"
      className="w-[134px] h-[94px] absolute left-[17px] top-[21px] object-none"
    />
  </div>
  <div
    className="w-[170px] h-[138px] absolute left-[135px] top-[785px] overflow-hidden rounded bg-neutral-100"
  >
    <img
      src="images/x.png"
      className="w-[122px] h-[106px] absolute left-[23px] top-[15px] object-none"
    />
  </div>
  <p className="absolute left-[905px] top-[323px] text-2xl font-semibold text-left text-black">
    Havic HV G-92 Gamepad
  </p>
  <p className="absolute left-[905px] top-[400px] text-2xl text-left text-black">$192.00</p>
  <div className="flex justify-start items-start absolute left-[905px] top-[363px] gap-4">
    <div className="flex justify-start items-start flex-grow-0 flex-shrink-0 relative gap-2">
      <div className="flex justify-start items-start flex-grow-0 flex-shrink-0 relative">
        <svg
          width="16"
          height="15"
          viewBox="0 0 16 15"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          className="flex-grow-0 flex-shrink-0"
          preserveAspectRatio="none"
        >
          <path
            d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
            fill="#FFAD33"
          ></path></svg
        ><svg
          width="16"
          height="15"
          viewBox="0 0 16 15"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          className="flex-grow-0 flex-shrink-0"
          preserveAspectRatio="none"
        >
          <path
            d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
            fill="#FFAD33"
          ></path></svg
        ><svg
          width="16"
          height="15"
          viewBox="0 0 16 15"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          className="flex-grow-0 flex-shrink-0"
          preserveAspectRatio="none"
        >
          <path
            d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
            fill="#FFAD33"
          ></path></svg
        ><svg
          width="16"
          height="15"
          viewBox="0 0 16 15"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          className="flex-grow-0 flex-shrink-0"
          preserveAspectRatio="none"
        >
          <path
            d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
            fill="#FFAD33"
          ></path></svg
        ><svg
          width="16"
          height="15"
          viewBox="0 0 16 15"
          fill="none"
          xmlns="http://www.w3.org/2000/svg"
          className="flex-grow-0 flex-shrink-0 opacity-25"
          preserveAspectRatio="none"
        >
          <path
            opacity="0.25"
            d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
            fill="black"
          ></path>
        </svg>
      </div>
      <p className="flex-grow-0 flex-shrink-0 opacity-50 text-sm text-left text-black">(150 Reviews)</p>
    </div>
    <div className="flex justify-start items-center flex-grow-0 flex-shrink-0 relative gap-4">
      <svg
        width="1"
        height="17"
        viewBox="0 0 1 17"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        className="flex-grow-0 flex-shrink-0 opacity-50"
        preserveAspectRatio="none"
      >
        <line opacity="0.5" x1="0.5" y1="0.5" x2="0.499999" y2="16.5" stroke="black"></line>
      </svg>
      <p className="flex-grow-0 flex-shrink-0 opacity-60 text-sm text-left text-[#0f6]">In Stock</p>
    </div>
  </div>
  <p className="w-[373px] absolute left-[905px] top-[448px] text-sm text-left text-black">
    PlayStation 5 Controller Skin High quality vinyl with air channel adhesive for easy bubble free
    install &#x26; mess free removal Pressure sensitive.
  </p>
  <div className="flex justify-start items-start absolute left-[906px] top-[559px] gap-6">
    <p className="flex-grow-0 flex-shrink-0 text-xl text-left text-black">Colours:</p>
    <div className="flex justify-start items-start flex-grow-0 flex-shrink-0 relative gap-2">
      <svg
        width="20"
        height="20"
        viewBox="0 0 20 20"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        className="flex-grow-0 flex-shrink-0 w-5 h-5"
        preserveAspectRatio="none"
      >
        <circle cx="10" cy="10" r="6" fill="#A0BCE0"></circle>
        <circle cx="10" cy="10" r="9" stroke="black" strokeWidth="2"></circle></svg
      ><svg
        width="20"
        height="20"
        viewBox="0 0 20 20"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        className="flex-grow-0 flex-shrink-0"
        preserveAspectRatio="xMidYMid meet"
      >
        <circle cx="10" cy="10" r="10" fill="#E07575"></circle>
      </svg>
    </div>
  </div>
  <div className="flex justify-start items-center absolute left-[906px] top-[603px] gap-6">
    <p className="flex-grow-0 flex-shrink-0 text-xl text-left text-black">Size:</p>
    <div className="flex justify-start items-start flex-grow-0 flex-shrink-0 relative gap-4">
      <div
        className="flex-grow-0 flex-shrink-0 w-8 h-8 relative overflow-hidden rounded border border-black/50"
      >
        <p
          className="w-[18px] h-5 absolute left-[7px] top-1.5 text-sm font-medium text-left text-black"
        >
          XS
        </p>
      </div>
      <div
        className="flex-grow-0 flex-shrink-0 w-8 h-8 relative overflow-hidden group rounded border border-black/50"
      >
        <p className="w-2 h-5 absolute left-3 top-1.5 text-sm font-medium border text-left text-black">S</p>
      </div>
      <div className="flex-grow-0 flex-shrink-0 w-8 h-8 relative overflow-hidden hover:bg-[#db4444] border border-black/50 rounded hover:text-neutral-50 ">
        <p className="w-3 h-5 absolute left-2.5 top-1.5 text-sm font-medium text-left hover:bg-[#db4444] text-black hover:text-neutral-50">
          M
        </p>
      </div>
      <div
        className="flex-grow-0 flex-shrink-0 w-8 h-8 relative overflow-hidden rounded border border-black/50"
      >
        <p className="w-1.5 h-5 absolute left-[13px] top-1.5 text-sm font-medium text-left text-black">
          L
        </p>
      </div>
      <div
        className="flex-grow-0 flex-shrink-0 w-8 h-8 relative overflow-hidden rounded border border-black/50"
      >
        <p className="w-4 h-5 absolute left-2 top-1.5 text-sm font-medium text-left text-black">XL</p>
      </div>
    </div>
  </div>
  <svg
    width="400"
    height="1"
    viewBox="0 0 400 1"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    className="w-[400px] h-0 absolute left-[905px] top-[535px]"
    preserveAspectRatio="none"
  >
    <g opacity="0.5"><line y1="0.5" x2="400" y2="0.5" stroke="black"></line></g>
  </svg>
  <div
    className="flex justify-start items-start w-[159px] h-11 absolute left-[906px] top-[659px] space-x-[-1px]"
  >
    <svg
      width="40"
      height="44"
      viewBox="0 0 40 44"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className="flex-grow-0 flex-shrink-0 w-10 h-11 relative"
      preserveAspectRatio="none"
    >
      <path
        d="M0.5 4C0.5 2.067 2.067 0.5 4 0.5H39.5V43.5H4C2.067 43.5 0.5 41.933 0.5 40V4Z"
        stroke="black"
        strokeOpacity="0.5"
      ></path>
      <path d="M28 22H12" stroke="black" strokeWidth="1.5" strokeLinecap="round"></path>
    </svg>
    <div
      className="flex-grow-0 flex-shrink-0 w-20 h-11 group relative overflow-hidden border border-black/50"
    >
      <p className="absolute left-[34px] top-2 text-xl font-medium group text-left text-black">2</p>
    </div>
    <svg
      width="41"
      height="44"
      viewBox="0 0 41 44"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className="flex-grow-0 flex-shrink-0 w-[41px] h-11 relative"
      preserveAspectRatio="none"
    >
      <path
      className='group:hover:bg-white'
        d="M0 0H37C39.2091 0 41 1.79086 41 4V40C41 42.2091 39.2091 44 37 44H0V0Z"
        fill="#DB4444"
      ></path>
      <path
        d="M21 30V22M21 22V14M21 22H29M21 22H13"
        stroke="white"
        strokeWidth="1.5"
        strokeLinecap="round"
      ></path>
    </svg>
  </div>
  <div
    className="flex justify-center items-center absolute left-[1081px] top-[659px] gap-2.5 px-12 py-2.5 rounded group border border-black/50 hover:bg-[#db4444]"
  >
    <p className="flex-grow-0 flex-shrink-0 text-base font-medium text-left group-hover:text-neutral-50 text-black">Buy Now</p>
  </div>
  <svg
    width="42"
    height="42"
    viewBox="0 0 42 42"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    className="w-10 h-10 absolute left-[1265px] top-[661px]"
    preserveAspectRatio="none"
  >
    <rect
      x="0.5"
      y="0.5"
      width="41"
      height="41"
      rx="4.5"
      stroke="black"
      strokeOpacity="0.5"
    ></rect>
    <path
      d="M16 12C13.239 12 11 14.216 11 16.95C11 19.157 11.875 24.395 20.488 29.69C20.6423 29.7839 20.8194 29.8335 21 29.8335C21.1806 29.8335 21.3577 29.7839 21.512 29.69C30.125 24.395 31 19.157 31 16.95C31 14.216 28.761 12 26 12C23.239 12 21 15 21 15C21 15 18.761 12 16 12Z"
      stroke="black"
      strokeWidth="1.5"
      strokeLinecap="round"
      strokeLinejoin="round"
    ></path>
  </svg>
  <div
    className="w-[399px] h-[180px] absolute left-[906px] top-[743px] overflow-hidden rounded border border-black/50"
  >
    <svg
      width="399"
      height="1"
      viewBox="0 0 399 1"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      className="w-[399px] h-0 absolute left-0 top-[90px]"
      preserveAspectRatio="none"
    >
      <g opacity="0.5"><line y1="0.5" x2="399" y2="0.5" stroke="black"></line></g>
    </svg>
    <div className="flex justify-start items-center absolute left-4 top-6 gap-4">
      <svg
        width="40"
        height="40"
        viewBox="0 0 40 40"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        className="flex-grow-0 flex-shrink-0 w-10 h-10 relative"
        preserveAspectRatio="none"
      >
        <g clipPath="url(#clip0_261_4843)">
          <path
            d="M11.6673 31.6667C13.5083 31.6667 15.0007 30.1743 15.0007 28.3333C15.0007 26.4924 13.5083 25 11.6673 25C9.82637 25 8.33398 26.4924 8.33398 28.3333C8.33398 30.1743 9.82637 31.6667 11.6673 31.6667Z"
            stroke="black"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
          ></path>
          <path
            d="M28.3333 31.6667C30.1743 31.6667 31.6667 30.1743 31.6667 28.3333C31.6667 26.4924 30.1743 25 28.3333 25C26.4924 25 25 26.4924 25 28.3333C25 30.1743 26.4924 31.6667 28.3333 31.6667Z"
            stroke="black"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
          ></path>
          <path
            d="M8.33398 28.3335H7.00065C5.89608 28.3335 5.00065 27.4381 5.00065 26.3335V21.6668M3.33398 8.3335H19.6673C20.7719 8.3335 21.6673 9.22893 21.6673 10.3335V28.3335M15.0007 28.3335H25.0007M31.6673 28.3335H33.0007C34.1052 28.3335 35.0007 27.4381 35.0007 26.3335V18.3335M35.0007 18.3335H21.6673M35.0007 18.3335L30.5833 10.9712C30.2218 10.3688 29.5708 10.0002 28.8683 10.0002H21.6673"
            stroke="black"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
          ></path>
          <path
            d="M8 28H6.66667C5.5621 28 4.66667 27.1046 4.66667 26V21.3333M3 8H19.3333C20.4379 8 21.3333 8.89543 21.3333 10V28M15 28H24.6667M32 28H32.6667C33.7712 28 34.6667 27.1046 34.6667 26V18M34.6667 18H21.3333M34.6667 18L30.2493 10.6377C29.8878 10.0353 29.2368 9.66667 28.5343 9.66667H21.3333"
            stroke="black"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
          ></path>
          <path
            d="M5 11.8182H11.6667"
            stroke="black"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
          ></path>
          <path
            d="M1.81836 15.4545H8.48503"
            stroke="black"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
          ></path>
          <path
            d="M5 19.0909H11.6667"
            stroke="black"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
          ></path>
        </g>
        <defs>
          <clipPath id="clip0_261_4843"><rect width="40" height="40" fill="white"></rect></clipPath>
        </defs>
      </svg>
      <div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 relative gap-2">
        <p className="flex-grow-0 flex-shrink-0 text-base font-medium text-left text-black">
          Free Delivery
        </p>
        <p className="flex-grow-0 flex-shrink-0 text-xs font-medium text-left text-black">
          Enter your postal code for Delivery Availability
        </p>
      </div>
    </div>
    <div className="flex justify-start items-center absolute left-4 top-[106px] gap-4">
      <svg
        width="40"
        height="40"
        viewBox="0 0 40 40"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        className="flex-grow-0 flex-shrink-0 w-10 h-10 relative"
        preserveAspectRatio="none"
      >
        <g clipPath="url(#clip0_261_4865)">
          <path
            d="M33.3327 18.3334C32.9251 15.4004 31.5645 12.6828 29.4604 10.5992C27.3564 8.51557 24.6256 7.18155 21.6888 6.80261C18.752 6.42366 15.7721 7.02082 13.208 8.5021C10.644 9.98337 8.6381 12.2666 7.49935 15M6.66602 8.33335V15H13.3327"
            stroke="black"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
          ></path>
          <path
            d="M6.66602 21.6667C7.07361 24.5997 8.43423 27.3173 10.5383 29.4009C12.6423 31.4845 15.3731 32.8185 18.3099 33.1974C21.2467 33.5764 24.2266 32.9792 26.7907 31.4979C29.3547 30.0167 31.3606 27.7335 32.4994 25M33.3327 31.6667V25H26.666"
            stroke="black"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
          ></path>
        </g>
        <defs>
          <clipPath id="clip0_261_4865"><rect width="40" height="40" fill="white"></rect></clipPath>
        </defs>
      </svg>
      <div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 relative gap-2">
        <p className="flex-grow-0 flex-shrink-0 text-base font-medium text-left text-black">
          Return Delivery
        </p>
        <p className="flex-grow-0 flex-shrink-0 text-xs font-medium text-left text-black">
          Free 30 Days Delivery Returns. Details
        </p>
      </div>
    </div>
  </div>
  <div
    className="flex flex-col justify-start items-start absolute left-[135px] top-[1063px] gap-[60px]"
  >
    <div className="flex justify-start items-center flex-grow-0 flex-shrink-0 relative gap-4">
      <div className="flex-grow-0 flex-shrink-0 w-5 h-10 relative">
        <div className="w-5 h-10 absolute left-[-1px] top-[-1px] rounded bg-[#db4444]"></div>
      </div>
      <p className="flex-grow-0 flex-shrink-0 text-base font-semibold text-left text-[#db4444]">
        Related Item
      </p>
    </div>
    <div className="flex justify-start items-start flex-grow-0 flex-shrink-0 gap-[30px]">
      <div className="flex flex-col justify-start items-start group flex-grow-0 flex-shrink-0 relative gap-4">



        <div
          className="flex-grow-0 flex-shrink-0 group w-[270px] h-[250px] relative overflow-hidden rounded bg-neutral-100"
        >
          <div
            className="flex justify-center items-center absolute left-3 top-3 gap-2.5 px-3 py-1 rounded bg-[#db4444]"
          >
            <p className="flex-grow-0 flex-shrink-0 text-xs text-left text-neutral-50">-40%</p>
          </div>
          <div
            className="w-[270px] h-[41px] opacity-0 group-hover:opacity-100 absolute left-[-1px] top-52 rounded-bl rounded-br bg-black"
          ></div>
           <Link href="/Cart">
              <p className="absolute left-[87px] top-[217px] text-base font-medium opacity-0 group-hover:opacity-100 text-left text-white">Add To Cart</p>
              </Link>
          <div className="flex flex-col justify-start items-start absolute left-56 top-3 gap-2">
            <svg
              width="34"
              height="34"
              viewBox="0 0 34 34"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="flex-grow-0 flex-shrink-0 w-[34px] h-[34px] relative"
              preserveAspectRatio="none"
            >
              <circle cx="17" cy="17" r="17" fill="white"></circle>
              <path
                d="M13 10C10.7912 10 9 11.7396 9 13.8859C9 15.6185 9.7 19.7305 16.5904 23.8873C16.7138 23.961 16.8555 24 17 24C17.1445 24 17.2862 23.961 17.4096 23.8873C24.3 19.7305 25 15.6185 25 13.8859C25 11.7396 23.2088 10 21 10C18.7912 10 17 12.3551 17 12.3551C17 12.3551 15.2088 10 13 10Z"
                stroke="black"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></path></svg
            ><svg
              width="34"
              height="34"
              viewBox="0 0 34 34"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="flex-grow-0 flex-shrink-0 w-[34px] h-[34px] relative"
              preserveAspectRatio="none"
            >
              <circle cx="17" cy="17" r="17" fill="white"></circle>
              <path
                d="M26.2565 15.962C26.7305 16.582 26.7305 17.419 26.2565 18.038C24.7635 19.987 21.1815 24 16.9995 24C12.8175 24 9.23552 19.987 7.74252 18.038C7.51191 17.7411 7.38672 17.3759 7.38672 17C7.38672 16.6241 7.51191 16.2589 7.74252 15.962C9.23552 14.013 12.8175 10 16.9995 10C21.1815 10 24.7635 14.013 26.2565 15.962V15.962Z"
                stroke="black"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></path>
              <path
                d="M17 20C18.6569 20 20 18.6569 20 17C20 15.3431 18.6569 14 17 14C15.3431 14 14 15.3431 14 17C14 18.6569 15.3431 20 17 20Z"
                stroke="black"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></path>
            </svg>
          
          </div>
          <div className="w-[190px] h-[180px] absolute left-10 top-[35px] overflow-hidden">
            <img
              src="images/i.png"
              className="w-[172px] h-[152px] absolute left-2 top-[13px] object-none"
            />
          </div>
        </div>
        <div
          className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 relative gap-2"
        >
          <p className="flex-grow-0 flex-shrink-0 text-base font-medium text-left text-black">
            HAVIT HV-G92 Gamepad
          </p>
          <div className="flex justify-start items-start flex-grow-0 flex-shrink-0 relative gap-3">
            <p className="flex-grow-0 flex-shrink-0 text-base font-medium text-left text-[#db4444]">
              $120
            </p>
            <p
              className="flex-grow-0 flex-shrink-0 opacity-50 text-base font-medium text-left text-black"
            >
              $160
            </p>
          </div>
          <div className="flex justify-start items-start flex-grow-0 flex-shrink-0 relative gap-2">
            <div className="flex justify-start items-start flex-grow-0 flex-shrink-0 relative">
              <svg
                width="16"
                height="15"
                viewBox="0 0 16 15"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0"
                preserveAspectRatio="none"
              >
                <path
                  d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
                  fill="#FFAD33"
                ></path></svg
              ><svg
                width="16"
                height="15"
                viewBox="0 0 16 15"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0"
                preserveAspectRatio="none"
              >
                <path
                  d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
                  fill="#FFAD33"
                ></path></svg
              ><svg
                width="16"
                height="15"
                viewBox="0 0 16 15"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0"
                preserveAspectRatio="none"
              >
                <path
                  d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
                  fill="#FFAD33"
                ></path></svg
              ><svg
                width="16"
                height="15"
                viewBox="0 0 16 15"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0"
                preserveAspectRatio="none"
              >
                <path
                  d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
                  fill="#FFAD33"
                ></path></svg
              ><svg
                width="16"
                height="15"
                viewBox="0 0 16 15"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0"
                preserveAspectRatio="none"
              >
                <path
                  d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
                  fill="#FFAD33"
                ></path>
              </svg>
            </div>
            <p
              className="flex-grow-0 flex-shrink-0 w-8 h-5 opacity-50 text-sm font-semibold text-left text-black"
            >
              (88)
            </p>
          </div>
        </div>
      </div>
      <div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 relative gap-4">
        <div
          className="flex-grow-0 flex-shrink-0 group w-[270px] h-[250px] relative overflow-hidden rounded bg-neutral-100"
        >
          <div
            className="flex justify-center items-center absolute left-3 top-3 gap-2.5 px-3 py-1 rounded bg-[#db4444]"
          >
            <p className="flex-grow-0 flex-shrink-0 text-xs text-left text-neutral-50">-35%</p>
          </div>
          <div
            className="w-[270px] h-[41px] opacity-0 group-hover:opacity-100 absolute left-[-1px] top-52 rounded-bl rounded-br bg-black"
          ></div>
           <Link href="/Cart">
              <p className="absolute left-[87px] top-[217px] text-base font-medium opacity-0 group-hover:opacity-100 text-left text-white">Add To Cart</p>
              </Link>
          <div className="flex flex-col justify-start items-start absolute left-56 top-3 gap-2">
            <svg
              width="34"
              height="34"
              viewBox="0 0 34 34"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="flex-grow-0 flex-shrink-0 w-[34px] h-[34px] relative"
              preserveAspectRatio="none"
            >
              <circle cx="17" cy="17" r="17" fill="white"></circle>
              <path
                d="M13 10C10.7912 10 9 11.7396 9 13.8859C9 15.6185 9.7 19.7305 16.5904 23.8873C16.7138 23.961 16.8555 24 17 24C17.1445 24 17.2862 23.961 17.4096 23.8873C24.3 19.7305 25 15.6185 25 13.8859C25 11.7396 23.2088 10 21 10C18.7912 10 17 12.3551 17 12.3551C17 12.3551 15.2088 10 13 10Z"
                stroke="black"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></path></svg
            ><svg
              width="34"
              height="34"
              viewBox="0 0 34 34"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="flex-grow-0 flex-shrink-0 w-[34px] h-[34px] relative"
              preserveAspectRatio="none"
            >
              <circle cx="17" cy="17" r="17" fill="white"></circle>
              <path
                d="M26.2565 15.962C26.7305 16.582 26.7305 17.419 26.2565 18.038C24.7635 19.987 21.1815 24 16.9995 24C12.8175 24 9.23552 19.987 7.74252 18.038C7.51191 17.7411 7.38672 17.3759 7.38672 17C7.38672 16.6241 7.51191 16.2589 7.74252 15.962C9.23552 14.013 12.8175 10 16.9995 10C21.1815 10 24.7635 14.013 26.2565 15.962V15.962Z"
                stroke="black"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></path>
              <path
                d="M17 20C18.6569 20 20 18.6569 20 17C20 15.3431 18.6569 14 17 14C15.3431 14 14 15.3431 14 17C14 18.6569 15.3431 20 17 20Z"
                stroke="black"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></path>
            </svg>
          </div>
          <div className="w-[190px] h-[180px] absolute left-10 top-[15px] overflow-hidden">
            <img
              src="images/j.png"
              className="w-[191px] h-[101px] absolute left-[-2px] top-[39px] object-none"
            />
          </div>
        </div>
        <div
          className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 relative gap-2"
        >
          <p className="flex-grow-0 flex-shrink-0 text-base font-medium text-left text-black">
            AK-900 Wired Keyboard
          </p>
          <div className="flex justify-start items-start flex-grow-0 flex-shrink-0 relative gap-3">
            <p className="flex-grow-0 flex-shrink-0 text-base font-medium text-left text-[#db4444]">
              $960
            </p>
            <p
              className="flex-grow-0 flex-shrink-0 opacity-50 text-base font-medium text-left text-black"
            >
              $1160
            </p>
          </div>
          <div className="flex justify-start items-start flex-grow-0 flex-shrink-0 relative gap-2">
            <div className="flex justify-start items-start flex-grow-0 flex-shrink-0 relative">
              <svg
                width="16"
                height="15"
                viewBox="0 0 16 15"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0"
                preserveAspectRatio="none"
              >
                <path
                  d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
                  fill="#FFAD33"
                ></path></svg
              ><svg
                width="16"
                height="15"
                viewBox="0 0 16 15"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0"
                preserveAspectRatio="none"
              >
                <path
                  d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
                  fill="#FFAD33"
                ></path></svg
              ><svg
                width="16"
                height="15"
                viewBox="0 0 16 15"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0"
                preserveAspectRatio="none"
              >
                <path
                  d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
                  fill="#FFAD33"
                ></path></svg
              ><svg
                width="16"
                height="15"
                viewBox="0 0 16 15"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0"
                preserveAspectRatio="none"
              >
                <path
                  d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
                  fill="#FFAD33"
                ></path></svg
              ><svg
                width="16"
                height="15"
                viewBox="0 0 16 15"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0 opacity-25"
                preserveAspectRatio="none"
              >
                <path
                  opacity="0.25"
                  d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
                  fill="black"
                ></path>
              </svg>
            </div>
            <p
              className="flex-grow-0 flex-shrink-0 w-8 h-5 opacity-50 text-sm font-semibold text-left text-black"
            >
              (75)
            </p>
          </div>
        </div>
      </div>
      <div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 relative gap-4">
        <div
          className="flex-grow-0 flex-shrink-0 w-[270px] h-[250px] group relative overflow-hidden rounded bg-neutral-100"
        >
          <div
            className="flex justify-center items-center absolute left-3 top-3 gap-2.5 px-3 py-1 rounded bg-[#db4444]"
          >
            <p className="flex-grow-0 flex-shrink-0 text-xs text-left text-neutral-50">-30%</p>
          </div>
            <div
            className="w-[270px] h-[41px] opacity-0 group-hover:opacity-100 absolute left-[-1px] top-52 rounded-bl rounded-br bg-black"
          ></div>
           <Link href="/Cart">
              <p className="absolute left-[87px] top-[217px] text-base font-medium opacity-0 group-hover:opacity-100 text-left text-white">Add To Cart</p>
              </Link>
          <div className="flex flex-col justify-start items-start absolute left-56 top-3 gap-2">
            <svg
              width="34"
              height="34"
              viewBox="0 0 34 34"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="flex-grow-0 flex-shrink-0 w-[34px] h-[34px] relative"
              preserveAspectRatio="none"
            >
              <circle cx="17" cy="17" r="17" fill="white"></circle>
              <path
                d="M13 10C10.7912 10 9 11.7396 9 13.8859C9 15.6185 9.7 19.7305 16.5904 23.8873C16.7138 23.961 16.8555 24 17 24C17.1445 24 17.2862 23.961 17.4096 23.8873C24.3 19.7305 25 15.6185 25 13.8859C25 11.7396 23.2088 10 21 10C18.7912 10 17 12.3551 17 12.3551C17 12.3551 15.2088 10 13 10Z"
                stroke="black"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></path></svg
            ><svg
              width="34"
              height="34"
              viewBox="0 0 34 34"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="flex-grow-0 flex-shrink-0 w-[34px] h-[34px] relative"
              preserveAspectRatio="none"
            >
              <circle cx="17" cy="17" r="17" fill="white"></circle>
              <path
                d="M26.2565 15.962C26.7305 16.582 26.7305 17.419 26.2565 18.038C24.7635 19.987 21.1815 24 16.9995 24C12.8175 24 9.23552 19.987 7.74252 18.038C7.51191 17.7411 7.38672 17.3759 7.38672 17C7.38672 16.6241 7.51191 16.2589 7.74252 15.962C9.23552 14.013 12.8175 10 16.9995 10C21.1815 10 24.7635 14.013 26.2565 15.962V15.962Z"
                stroke="black"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></path>
              <path
                d="M17 20C18.6569 20 20 18.6569 20 17C20 15.3431 18.6569 14 17 14C15.3431 14 14 15.3431 14 17C14 18.6569 15.3431 20 17 20Z"
                stroke="black"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></path>
            </svg>
          </div>
          <div className="w-[190px] h-[180px] absolute left-10 top-[35px] overflow-hidden">
            <img
              src="images/k.png"
              className="w-[170px] h-[129px] absolute left-[9px] top-[25px] object-none"
            />
          </div>
        </div>
        <div
          className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 relative gap-2"
        >
          <p className="flex-grow-0 flex-shrink-0 text-base font-medium text-left text-black">
            IPS LCD Gaming Monitor
          </p>
          <div className="flex justify-start items-start flex-grow-0 flex-shrink-0 relative gap-3">
            <p className="flex-grow-0 flex-shrink-0 text-base font-medium text-left text-[#db4444]">
              $370
            </p>
            <p
              className="flex-grow-0 flex-shrink-0 opacity-50 text-base font-medium text-left text-black"
            >
              $400
            </p>
          </div>
          <div className="flex justify-start items-start flex-grow-0 flex-shrink-0 relative gap-2">
            <div className="flex justify-start items-start flex-grow-0 flex-shrink-0 relative">
              <svg
                width="16"
                height="15"
                viewBox="0 0 16 15"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0"
                preserveAspectRatio="none"
              >
                <path
                  d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
                  fill="#FFAD33"
                ></path></svg
              ><svg
                width="16"
                height="15"
                viewBox="0 0 16 15"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0"
                preserveAspectRatio="none"
              >
                <path
                  d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
                  fill="#FFAD33"
                ></path></svg
              ><svg
                width="16"
                height="15"
                viewBox="0 0 16 15"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0"
                preserveAspectRatio="none"
              >
                <path
                  d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
                  fill="#FFAD33"
                ></path></svg
              ><svg
                width="16"
                height="15"
                viewBox="0 0 16 15"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0"
                preserveAspectRatio="none"
              >
                <path
                  d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
                  fill="#FFAD33"
                ></path></svg
              ><svg
                width="16"
                height="15"
                viewBox="0 0 16 15"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0"
                preserveAspectRatio="none"
              >
                <path
                  d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
                  fill="#FFAD33"
                ></path>
              </svg>
            </div>
            <p
              className="flex-grow-0 flex-shrink-0 w-8 h-5 opacity-50 text-sm font-semibold text-left text-black"
            >
              (99)
            </p>
          </div>
        </div>
      </div>
      <div className="flex flex-col justify-start  items-start flex-grow-0 flex-shrink-0 relative gap-4">
        <div
          className="flex-grow-0 flex-shrink-0 w-[270px] h-[250px]  group relative overflow-hidden rounded bg-neutral-100"
        >

<div
            className="w-[270px] h-[41px] opacity-0 group-hover:opacity-100 absolute left-[-1px] top-52 rounded-bl rounded-br bg-black"
          ></div>
           <Link href="/Cart">
              <p className="absolute left-[87px] top-[217px] text-base font-medium opacity-0 group-hover:opacity-100 text-left text-white">Add To Cart</p>
              </Link>
            
          <div className="flex flex-col justify-start items-start absolute left-56 top-3 gap-2">
            <svg
              width="34"
              height="34"
              viewBox="0 0 34 34"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="flex-grow-0 flex-shrink-0 w-[34px] h-[34px] relative"
              preserveAspectRatio="none"
            >
              <circle cx="17" cy="17" r="17" fill="white"></circle>
              <path
                d="M13 10C10.7912 10 9 11.7396 9 13.8859C9 15.6185 9.7 19.7305 16.5904 23.8873C16.7138 23.961 16.8555 24 17 24C17.1445 24 17.2862 23.961 17.4096 23.8873C24.3 19.7305 25 15.6185 25 13.8859C25 11.7396 23.2088 10 21 10C18.7912 10 17 12.3551 17 12.3551C17 12.3551 15.2088 10 13 10Z"
                stroke="black"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></path></svg
            ><svg
              width="34"
              height="34"
              viewBox="0 0 34 34"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
              className="flex-grow-0 flex-shrink-0 w-[34px] h-[34px] relative"
              preserveAspectRatio="none"
            >
              <circle cx="17" cy="17" r="17" fill="white"></circle>
              <path
                d="M26.2565 15.962C26.7305 16.582 26.7305 17.419 26.2565 18.038C24.7635 19.987 21.1815 24 16.9995 24C12.8175 24 9.23552 19.987 7.74252 18.038C7.51191 17.7411 7.38672 17.3759 7.38672 17C7.38672 16.6241 7.51191 16.2589 7.74252 15.962C9.23552 14.013 12.8175 10 16.9995 10C21.1815 10 24.7635 14.013 26.2565 15.962V15.962Z"
                stroke="black"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></path>
              <path
                d="M17 20C18.6569 20 20 18.6569 20 17C20 15.3431 18.6569 14 17 14C15.3431 14 14 15.3431 14 17C14 18.6569 15.3431 20 17 20Z"
                stroke="black"
                strokeWidth="1.5"
                strokeLinecap="round"
                strokeLinejoin="round"
              ></path>
            </svg>
          </div>
          
          <div className="w-[190px] h-[180px] absolute left-10 top-[35px] overflow-hidden">
            <img
              src="images/3.png"
              className="w-[191px] h-[95px] absolute left-[-1px] top-[46px] object-none"
            />
          </div>
        </div>
        <div
          className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 relative gap-2"
        >
          <p className="flex-grow-0 flex-shrink-0 text-base font-medium text-left text-black">
            RGB liquid CPU Cooler
          </p>
          <div className="flex justify-start items-start flex-grow-0 flex-shrink-0 relative gap-3">
            <p className="flex-grow-0 flex-shrink-0 text-base font-medium text-left text-[#db4444]">
              $160
            </p>
            <p
              className="flex-grow-0 flex-shrink-0 opacity-50 text-base font-medium text-left text-black"
            >
              $170
            </p>
          </div>
          <div className="flex justify-start items-start flex-grow-0 flex-shrink-0 relative gap-2">
            <div className="flex justify-start items-start flex-grow-0 flex-shrink-0 relative">
              <svg
                width="16"
                height="15"
                viewBox="0 0 16 15"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0"
                preserveAspectRatio="none"
              >
                <path
                  d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
                  fill="#FFAD33"
                ></path></svg
              ><svg
                width="16"
                height="15"
                viewBox="0 0 16 15"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0"
                preserveAspectRatio="none"
              >
                <path
                  d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
                  fill="#FFAD33"
                ></path></svg
              ><svg
                width="16"
                height="15"
                viewBox="0 0 16 15"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0"
                preserveAspectRatio="none"
              >
                <path
                  d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
                  fill="#FFAD33"
                ></path></svg
              ><svg
                width="16"
                height="15"
                viewBox="0 0 16 15"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0"
                preserveAspectRatio="none"
              >
                <path
                  d="M14.673 7.17173C15.7437 6.36184 15.1709 4.65517 13.8284 4.65517H11.3992C10.7853 4.65517 10.243 4.25521 10.0617 3.66868L9.33754 1.32637C8.9309 0.0110567 7.0691 0.0110564 6.66246 1.32637L5.93832 3.66868C5.75699 4.25521 5.21469 4.65517 4.60078 4.65517H2.12961C0.791419 4.65517 0.215919 6.35274 1.27822 7.16654L3.39469 8.78792C3.85885 9.1435 4.05314 9.75008 3.88196 10.3092L3.11296 12.8207C2.71416 14.1232 4.22167 15.1704 5.30301 14.342L7.14861 12.9281C7.65097 12.5432 8.34903 12.5432 8.85139 12.9281L10.6807 14.3295C11.7636 15.159 13.2725 14.1079 12.8696 12.8046L12.09 10.2827C11.9159 9.71975 12.113 9.10809 12.5829 8.75263L14.673 7.17173Z"
                  fill="#FFAD33"
                ></path></svg
              ><svg
                width="20"
                height="20"
                viewBox="0 0 20 20"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0 w-5 h-5 relative"
                preserveAspectRatio="none"
              >
                <path
                  d="M9.9992 1.83329C9.75009 1.83145 9.50558 1.90032 9.29405 2.0319C9.08252 2.16348 8.91266 2.35236 8.80419 2.57662L6.95253 6.32995L2.80919 6.93162C2.56275 6.96713 2.33117 7.07094 2.14069 7.23129C1.9502 7.39163 1.80842 7.60212 1.73141 7.83889C1.65439 8.07567 1.64521 8.32929 1.70491 8.57101C1.76461 8.81274 1.8908 9.03292 2.06919 9.20662L5.06919 12.1283L4.36086 16.255C4.31873 16.5001 4.346 16.7521 4.43959 16.9825C4.53318 17.213 4.68936 17.4126 4.89047 17.559C5.09159 17.7053 5.32963 17.7925 5.57769 17.8106C5.82574 17.8287 6.07393 17.7771 6.29419 17.6616L9.9992 15.7133V1.83329Z"
                  fill="#FFAD33"
                ></path>
                <path
                  opacity="0.25"
                  d="M10.0007 1.83595C10.2498 1.83404 10.5007 2.00001 10.5007 2.00001C10.5007 2.00001 11.0872 2.20807 11.1957 2.44195L13.0474 6.35634L17.1908 6.98383C17.4372 7.02086 17.6688 7.12912 17.8593 7.29635C18.0498 7.46358 18.1916 7.68309 18.2686 7.93003C18.3456 8.17697 18.3548 8.44147 18.2951 8.69357C18.2354 8.94567 18.1092 9.1753 17.9308 9.35645L14.9307 12.4035L15.6391 16.7072C15.6812 16.9629 15.654 17.2257 15.5604 17.466C15.4668 17.7064 15.3106 17.9146 15.1095 18.0672C14.9083 18.2198 14.6703 18.3107 14.4222 18.3296C14.1742 18.3486 13.926 18.2947 13.7057 18.1743L10.0007 16.1423V1.83595Z"
                  fill="black"
                ></path>
              </svg>
            </div>
            <p
              className="flex-grow-0 flex-shrink-0 w-8 h-5 opacity-50 text-sm font-semibold text-left text-black"
            >
              (65)
            </p>
          </div>
        </div>
      </div>
    </div>
  </div>
</div>



























































     
  <div className="w-[1440px] h-[440px] absolute left-0 top-[1653px] overflow-hidden bg-black">
    <div
      className="flex flex-col justify-start items-center absolute left-0 top-[376px] opacity-40 gap-4"
    >
      <svg
            width={1440}
            height={1}
            viewBox="0 0 1440 1"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className="flex-grow-0 flex-shrink-0 w-[1440px] h-0 relative"
            preserveAspectRatio="none"
          >
            <g opacity="0.5">
              <line opacity="0.4" y1="0.5" x2={1440} y2="0.5" stroke="white" />
            </g>
          </svg>
          <div className="flex justify-start items-center flex-grow-0 flex-shrink-0 opacity-60 gap-3">
            <div className="flex justify-start items-center flex-grow-0 flex-shrink-0 relative gap-1.5">
              <svg
                width={21}
                height={20}
                viewBox="0 0 21 20"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0 w-5 h-5 relative"
                preserveAspectRatio="none"
              >
                <path
                  d="M10.5 18.3332C15.1024 18.3332 18.8334 14.6022 18.8334 9.99984C18.8334 5.39746 15.1024 1.6665 10.5 1.6665C5.89765 1.6665 2.16669 5.39746 2.16669 9.99984C2.16669 14.6022 5.89765 18.3332 10.5 18.3332Z"
                  stroke="white"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
                <path
                  d="M13 8.14799C13 8.14799 11.9706 6.6665 10.2549 6.6665C8.53924 6.6665 7.16669 8.14799 7.16669 9.99984C7.16669 11.8517 8.53924 13.3332 10.2549 13.3332C11.9706 13.3332 13 11.8517 13 11.8517"
                  stroke="white"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
              </svg>
              <p className="flex-grow-0 flex-shrink-0 text-base text-left text-white">
                Copyright Rimel 2022. All right reserved
              </p>
            </div>
          </div>
        </div>
        <div className="flex justify-center items-start absolute left-[135px] top-20 gap-[87px]">
          <div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 gap-4">
            <div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 relative gap-6">
              <div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 relative gap-6">
                <div className="flex-grow-0 flex-shrink-0 w-[118px] h-6 relative">
                  <p className="absolute left-0 top-0 text-2xl font-bold text-left text-neutral-50">
                    Exclusive
                  </p>
                </div>
                <p className="flex-grow-0 flex-shrink-0 text-xl font-medium text-left text-neutral-50">
                  Subscribe
                </p>
              </div>
              <p className="flex-grow-0 flex-shrink-0 text-base text-left text-neutral-50">
                Get 10% off your first order
              </p>
            </div>
            <div className="flex justify-start items-center flex-grow-0 flex-shrink-0 w-[217px] relative gap-8 pl-0 py-3 rounded bg-black ">
              <input className=" text-black" placeholder="Enter your email"
              />
              <svg
                width={24}
                height={24}
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0 w-6 h-6 relative"
                preserveAspectRatio="none"
              >
                <path
                  d="M9.91199 11.9998H3.99999L2.02299 4.1348C2.01033 4.0891 2.00262 4.04216 1.99999 3.9948C1.97799 3.2738 2.77199 2.7738 3.45999 3.1038L22 11.9998L3.45999 20.8958C2.77999 21.2228 1.99599 20.7368 1.99999 20.0288C2.00201 19.9655 2.01313 19.9029 2.03299 19.8428L3.49999 14.9998"
                  stroke="#FAFAFA"
                  strokeWidth="1.5"
                  strokeLinecap="round"
                  strokeLinejoin="round"
                />
              </svg>
            </div>
          </div>
          <div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 relative gap-6">
            <p className="flex-grow-0 flex-shrink-0 text-xl font-medium text-left text-neutral-50">
              Support
            </p>
            <div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 relative gap-4">
              <p className="flex-grow-0 flex-shrink-0 w-[175px] text-base text-left text-neutral-50">
                Plaza Nazimabad, Karachi, Pakistan.
              </p>
              <p className="flex-grow-0 flex-shrink-0 text-base text-left text-neutral-50">
                mominshykh7@gmail.com
              </p>
              <p className="flex-grow-0 flex-shrink-0 text-base text-left text-neutral-50">
                +92 315 2309503
              </p>
            </div>
          </div>
          <div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 relative gap-6">
            <p className="flex-grow-0 flex-shrink-0 text-xl font-medium text-left text-neutral-50">
              Account
            </p>
            <div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 relative gap-4">
              <p className="flex-grow-0 flex-shrink-0 text-base text-left text-neutral-50">
                My Account
              </p>
              <p className="flex-grow-0 flex-shrink-0 text-base text-left text-neutral-50">
                Login / Register
              </p>
              <p className="flex-grow-0 flex-shrink-0 text-base text-left text-neutral-50">Cart</p>
              <p className="flex-grow-0 flex-shrink-0 text-base text-left text-neutral-50">Wishlist</p>
              <p className="flex-grow-0 flex-shrink-0 text-base text-left text-neutral-50">Shop</p>
            </div>
          </div>
          <div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 relative gap-6">
            <p className="flex-grow-0 flex-shrink-0 text-xl font-medium text-left text-neutral-50">
              Quick Link
            </p>
            <div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 relative gap-4">
              <p className="flex-grow-0 flex-shrink-0 text-base text-left text-neutral-50">
                Privacy Policy
              </p>
              <p className="flex-grow-0 flex-shrink-0 text-base text-left text-neutral-50">
                Terms Of Use
              </p>
              <p className="flex-grow-0 flex-shrink-0 text-base text-left text-neutral-50">FAQ</p>
              <p className="flex-grow-0 flex-shrink-0 text-base text-left text-neutral-50">Contact</p>
            </div>
          </div>
          <div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 gap-6">
            <div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 relative gap-6">
              <p className="flex-grow-0 flex-shrink-0 text-xl font-medium text-left text-neutral-50">
                Download App
              </p>
              <div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 relative gap-2">
                <p className="flex-grow-0 flex-shrink-0 opacity-70 text-xs font-medium text-left text-neutral-50">
                  Save $3 with App New User Only
                </p>
                <div className="flex justify-start items-center flex-grow-0 flex-shrink-0 relative gap-2">
                  <div className="flex-grow-0 flex-shrink-0 w-20 h-20 relative overflow-hidden bg-black">
                    <img
                      src="images/code.png"
                      className="w-[76px] h-[76px] absolute left-[0.25px] top-[0.25px] object-cover border-[2.5px] border-white"
                    />
                  </div>
                  <div className="flex flex-col justify-start items-start flex-grow-0 flex-shrink-0 relative gap-1">
                    <div className="flex-grow-0 flex-shrink-0 w-[110px] h-10 relative overflow-hidden bg-[#030406]">
                      <img
                        src="images/playstore.png"
                        className="w-[104px] h-[30px] absolute left-[2.2px] top-[4.2px] rounded object-none border-[0.6px] border-neutral-50"
                      />
                      <svg
                        width={3}
                        height={3}
                        viewBox="0 0 3 3"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                        className="absolute left-[98.39px] top-[21.24px]"
                        preserveAspectRatio="none"
                      >
                        <path
                          d="M1.9545 1.24463C1.83328 1.26195 1.72141 1.7928 1.64483 1.91533C1.57459 2.02771 1.50379 2.12399 1.39429 2.19699"
                          stroke="white"
                          strokeLinecap="round"
                        />
                      </svg>
                      <svg
                        width={3}
                        height={6}
                        viewBox="0 0 3 6"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                        className="absolute left-[97.19px] top-[20.96px]"
                        preserveAspectRatio="none"
                      >
                        <path
                          d="M2.45028 0.964355C2.3818 1.10131 2.31512 1.23484 2.24487 1.36896C2.20438 1.44625 2.21093 1.54347 2.16862 1.62105C2.12388 1.70308 2.06189 1.72599 2.05813 1.83891C2.05582 1.90823 2.05813 1.97806 2.05813 2.04744C2.05813 2.40502 2.08984 2.76867 1.91963 3.08072C1.87766 3.15768 1.87023 3.26773 1.84649 3.35149C1.82526 3.42645 1.75807 3.5057 1.75001 3.57558C1.73727 3.68605 1.612 3.85532 1.5695 3.96462C1.52806 4.07117 1.54269 4.08634 1.4699 4.17626C1.39728 4.26597 1.31977 4.35948 1.25204 4.45325C1.17139 4.56493 1.1898 4.75322 1.1898 4.88586"
                          stroke="white"
                          strokeLinecap="round"
                        />
                      </svg>
                      <svg
                        width={2}
                        height={2}
                        viewBox="0 0 2 2"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                        className="absolute left-[93.91px] top-[21.24px]"
                        preserveAspectRatio="none"
                      >
                        <path
                          d="M1.22072 1.30045C1.2101 1.2049 0.984506 1.24443 0.912598 1.24443"
                          stroke="white"
                          strokeLinecap="round"
                        />
                      </svg>
                      <svg
                        width={4}
                        height={4}
                        viewBox="0 0 4 4"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                        className="absolute left-[97.39px] top-[21.71px]"
                        preserveAspectRatio="none"
                      >
                        <path
                          d="M2.55529 0.708008C2.55343 0.751718 2.53573 0.794429 2.52628 0.836968C2.52125 0.859588 2.51811 0.883291 2.51016 0.905133C2.50388 0.922364 2.49302 0.93747 2.48989 0.955796C2.48636 0.976467 2.47609 0.991763 2.47147 1.01153C2.46705 1.03041 2.4688 1.04942 2.46732 1.06864C2.46095 1.15144 2.4641 1.23517 2.4641 1.31827C2.4641 1.33613 2.46653 1.35642 2.46318 1.374C2.45865 1.39778 2.4472 1.41721 2.43923 1.43986C2.42325 1.48527 2.37958 1.52384 2.36968 1.57112C2.35387 1.64667 2.32239 1.71942 2.28816 1.78989C2.26619 1.83512 2.25681 1.88791 2.23197 1.93175C2.20119 1.98607 2.17734 2.05262 2.13087 2.09664C2.108 2.1183 2.09591 2.15534 2.07561 2.17954C2.05731 2.20133 2.03384 2.21788 2.01504 2.23918C2.00718 2.2481 2.00213 2.25914 1.99408 2.26843C1.98664 2.27702 1.98444 2.29041 1.98349 2.30136C1.98229 2.31521 1.97538 2.31978 1.97152 2.33268C1.96206 2.36418 1.96427 2.39268 1.94434 2.4218C1.92909 2.44409 1.91784 2.47558 1.89828 2.49411C1.87291 2.51815 1.86416 2.55655 1.8391 2.583C1.82227 2.60077 1.81778 2.62658 1.80295 2.6461C1.78362 2.67153 1.77539 2.70188 1.75505 2.72716C1.72309 2.76686 1.68658 2.80771 1.64658 2.83954C1.63798 2.84638 1.62017 2.86764 1.60812 2.86764C1.58357 2.86764 1.56512 2.80975 1.55838 2.79256C1.54277 2.75271 1.5383 2.71 1.53144 2.66798C1.5284 2.64935 1.51259 2.6129 1.5296 2.5959C1.55085 2.57464 1.56615 2.55002 1.58556 2.52819C1.60336 2.50817 1.6294 2.49806 1.64589 2.47776C1.66575 2.45332 1.67216 2.42821 1.68642 2.39969C1.69447 2.3836 1.69392 2.3654 1.70162 2.34903C1.71012 2.33096 1.72251 2.30882 1.72626 2.28916C1.72952 2.27205 1.73997 2.24726 1.74883 2.2325C1.76439 2.20656 1.78225 2.18178 1.79581 2.15467C1.80141 2.14347 1.79905 2.13231 1.80179 2.12059C1.80529 2.10565 1.81514 2.09263 1.8193 2.07729C1.83665 2.01332 1.87701 1.95973 1.89529 1.89675C1.90247 1.87201 1.91741 1.85271 1.92431 1.82628C1.92872 1.80937 1.94089 1.79753 1.94503 1.78022C1.9476 1.76947 1.95139 1.75659 1.95609 1.7466C1.96655 1.72437 1.96542 1.69805 1.9699 1.67383C1.97364 1.65367 1.98091 1.6337 1.98648 1.61396C1.99239 1.59304 1.99255 1.57149 1.998 1.5504C2.00406 1.52689 2.00134 1.50188 2.00491 1.47809C2.00788 1.4583 2.01681 1.42343 2.02886 1.40762C2.05107 1.37847 2.05129 1.32475 2.06432 1.29063C2.07777 1.25542 2.08021 1.21687 2.09426 1.18148C2.10229 1.16124 2.10345 1.13991 2.11176 1.11976C2.11929 1.10151 2.13038 1.07473 2.12834 1.05436C2.12819 1.05284 2.09942 1.09626 2.0938 1.10502C2.08582 1.11746 2.08334 1.13239 2.07676 1.14555C2.06976 1.15954 2.05959 1.17158 2.05189 1.18516C2.0306 1.22271 2.01136 1.2642 1.98602 1.29892C1.97425 1.31506 1.96061 1.339 1.95816 1.35857C1.95543 1.38043 1.95283 1.4035 1.94964 1.42581C1.94309 1.47166 1.91694 1.51482 1.91694 1.56214C1.91694 1.59223 1.91694 1.62232 1.91694 1.65241C1.91694 1.67532 1.90753 1.68837 1.90427 1.70952C1.90247 1.72122 1.89802 1.73233 1.89621 1.74407C1.8935 1.76167 1.8788 1.77361 1.87341 1.79035C1.86936 1.80296 1.8707 1.82106 1.86282 1.83158C1.85484 1.84222 1.85241 1.85191 1.84854 1.86428C1.83864 1.89597 1.8345 1.93068 1.82022 1.96054C1.80426 1.99391 1.78155 2.02115 1.77186 2.05749C1.76789 2.07235 1.76066 2.08088 1.75366 2.09433C1.7473 2.10658 1.748 2.12052 1.74215 2.13279C1.73174 2.15466 1.71669 2.1725 1.71406 2.1975C1.71273 2.2101 1.70872 2.22011 1.70553 2.23204C1.69705 2.26387 1.68613 2.29569 1.67606 2.32715C1.66151 2.37262 1.63082 2.41235 1.62701 2.46187C1.62403 2.50061 1.62046 2.53936 1.61641 2.57816C1.6126 2.6148 1.59512 2.64446 1.58786 2.67995C1.58447 2.69654 1.58166 2.71098 1.57105 2.72463C1.5625 2.73562 1.56118 2.75198 1.55216 2.76401C1.52207 2.80413 1.47959 2.82627 1.43955 2.85474C1.41952 2.86899 1.39547 2.8884 1.3905 2.91323"
                          stroke="white"
                          strokeLinecap="round"
                        />
                      </svg>
                    </div>
                    <div className="flex-grow-0 flex-shrink-0 w-[110px] h-10 relative overflow-hidden bg-black">
                      <img
                        src="images/appstore.png"
                        className="w-[104px] h-[34px] absolute left-[2.2px] top-[2.2px] rounded object-none border-[0.6px] border-white"
                      />
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="flex justify-start items-start flex-grow-0 flex-shrink-0 relative gap-6">
              <svg
                width={24}
                height={24}
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0 w-6 h-6 relative"
                preserveAspectRatio="xMidYMid meet"
              >
                <path
                  d="M13 10H17.5L17 12H13V21H11V12H7V10H11V8.128C11 6.345 11.186 5.698 11.534 5.046C11.875 4.40181 12.4018 3.87501 13.046 3.534C13.698 3.186 14.345 3 16.128 3C16.65 3 17.108 3.05 17.5 3.15V5H16.128C14.804 5 14.401 5.078 13.99 5.298C13.686 5.46 13.46 5.686 13.298 5.99C13.078 6.401 13 6.804 13 8.128V10Z"
                  fill="white"
                />
              </svg>
              <svg
                width={24}
                height={24}
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0 w-6 h-6 relative"
                preserveAspectRatio="none"
              >
                <g clipPath="url(#clip0_34802_1563)">
                  <path
                    d="M12.905 8.84651L12.905 8.84646C12.9194 8.06035 13.2418 7.3113 13.8028 6.76049C14.3639 6.20969 15.1188 5.90116 15.905 5.90129L12.905 8.84651ZM12.905 8.84651L12.877 10.4213M12.905 8.84651L12.877 10.4213M4.75811 7.80857L4.89001 7.91846C6.76679 9.48211 8.71781 10.4182 10.7495 10.6952C10.7495 10.6952 10.7495 10.6952 10.7495 10.6952L12.3104 10.9072L4.75811 7.80857ZM4.75811 7.80857L4.72759 7.97751M4.75811 7.80857L4.72759 7.97751M4.72759 7.97751C4.42576 9.64819 4.5683 11.0709 5.1479 12.3018C5.72718 13.532 6.73827 14.5605 8.15577 15.4519L8.15579 15.452M4.72759 7.97751L8.15579 15.452M8.15579 15.452L9.90279 16.55L9.954 16.4685M8.15579 15.452L9.954 16.4685M9.954 16.4685L9.90279 16.55C9.97196 16.5934 10.0294 16.6532 10.0702 16.724C10.1109 16.7948 10.1337 16.8745 10.1365 16.9562C10.1392 17.0378 10.122 17.1189 10.0862 17.1924C10.0504 17.2658 9.99716 17.3294 9.93112 17.3775L9.93101 17.3775M9.954 16.4685L9.93101 17.3775M9.93101 17.3775L8.33901 18.5405L8.11542 18.7039M9.93101 17.3775L8.11542 18.7039M8.11542 18.7039L8.39178 18.7211M8.11542 18.7039L8.39178 18.7211M8.39178 18.7211C9.3449 18.7805 10.2529 18.7385 11.0095 18.5884L11.0096 18.5884M8.39178 18.7211L11.0096 18.5884M11.0096 18.5884C13.3886 18.1134 15.3745 16.9794 16.7652 15.2211M11.0096 18.5884L16.7652 15.2211M12.877 10.4213C12.8757 10.4918 12.8594 10.5612 12.8293 10.625C12.7993 10.6887 12.7561 10.7454 12.7026 10.7912C12.649 10.8371 12.5864 10.8712 12.5188 10.8911C12.4513 10.9111 12.3803 10.9166 12.3105 10.9072L12.877 10.4213ZM16.7652 15.2211C18.1557 13.463 18.945 11.0883 18.945 8.14229M16.7652 15.2211L18.945 8.14229M18.945 8.14229C18.945 7.99668 18.8714 7.78474 18.744 7.55722M18.945 8.14229L18.744 7.55722M18.744 7.55722C18.6142 7.32559 18.4215 7.06508 18.1673 6.82049M18.744 7.55722L18.1673 6.82049M18.1673 6.82049C17.6587 6.33088 16.8999 5.90129 15.905 5.90129L18.1673 6.82049ZM20.4978 5.53842C20.8818 5.48388 21.3285 5.34345 21.916 5.01105C21.6101 6.49526 21.4321 7.16764 20.7642 8.08336L20.745 8.10969V8.14229C20.745 11.9415 19.578 14.7567 17.8258 16.7397C16.0726 18.7238 13.7277 19.8813 11.3624 20.3532C9.74529 20.6759 7.7544 20.5728 5.99643 20.2106C5.11813 20.0296 4.30077 19.7846 3.61983 19.4974C3.03727 19.2517 2.56009 18.9775 2.22956 18.6904C2.66065 18.6482 3.4114 18.5535 4.24366 18.3598C5.24355 18.1272 6.37173 17.7494 7.20306 17.141L7.31918 17.056L7.19904 16.9768C7.15724 16.9492 7.11178 16.9196 7.06301 16.8879C6.30477 16.3938 4.74648 15.3786 3.73155 13.5166C2.66714 11.5637 2.19257 8.66295 3.91362 4.42592C5.57889 6.34347 7.2726 7.66001 8.99504 8.3668L8.99505 8.36681C9.57662 8.60536 9.94255 8.72373 10.2318 8.79141C10.4509 8.84265 10.6261 8.86463 10.8117 8.88794C10.8703 8.89529 10.93 8.90278 10.9924 8.91135L11.2872 8.95189L11.1059 8.77077C11.131 7.8414 11.4254 6.93895 11.9539 6.17331C12.4904 5.39606 13.2442 4.79434 14.1211 4.4435C14.9979 4.09265 15.9588 4.00828 16.8833 4.20093C17.8079 4.39359 18.6551 4.85471 19.3189 5.52657L19.3485 5.55658L19.3907 5.55628C19.4934 5.55556 19.5972 5.55908 19.7036 5.56269C19.9483 5.57098 20.2068 5.57974 20.4978 5.53842Z"
                    fill="white"
                    stroke="black"
                    strokeWidth="0.2"
                  />
                </g>
                <defs>
                  <clipPath id="clip0_34802_1563">
                    <rect width={24} height={24} fill="white" />
                  </clipPath>
                </defs>
              </svg>
              <svg
                width={24}
                height={24}
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0 w-6 h-6 relative"
                preserveAspectRatio="none"
              >
                <path
                  d="M17 3H7C5.93913 3 4.92172 3.42143 4.17157 4.17157C3.42143 4.92172 3 5.93913 3 7V17C3 18.0609 3.42143 19.0783 4.17157 19.8284C4.92172 20.5786 5.93913 21 7 21H17C18.0609 21 19.0783 20.5786 19.8284 19.8284C20.5786 19.0783 21 18.0609 21 17V7C21 5.93913 20.5786 4.92172 19.8284 4.17157C19.0783 3.42143 18.0609 3 17 3Z"
                  stroke="white"
                  strokeWidth="1.5"
                  strokeLinejoin="round"
                />
                <path
                  d="M12 16C13.0609 16 14.0783 15.5786 14.8284 14.8284C15.5786 14.0783 16 13.0609 16 12C16 10.9391 15.5786 9.92172 14.8284 9.17157C14.0783 8.42143 13.0609 8 12 8C10.9391 8 9.92172 8.42143 9.17157 9.17157C8.42143 9.92172 8 10.9391 8 12C8 13.0609 8.42143 14.0783 9.17157 14.8284C9.92172 15.5786 10.9391 16 12 16V16Z"
                  stroke="white"
                  strokeWidth="1.5"
                  strokeLinejoin="round"
                />
                <path
                  d="M17.5 7.5C17.7652 7.5 18.0196 7.39464 18.2071 7.20711C18.3946 7.01957 18.5 6.76522 18.5 6.5C18.5 6.23478 18.3946 5.98043 18.2071 5.79289C18.0196 5.60536 17.7652 5.5 17.5 5.5C17.2348 5.5 16.9804 5.60536 16.7929 5.79289C16.6054 5.98043 16.5 6.23478 16.5 6.5C16.5 6.76522 16.6054 7.01957 16.7929 7.20711C16.9804 7.39464 17.2348 7.5 17.5 7.5Z"
                  fill="white"
                />
              </svg>
              <svg
                width={24}
                height={24}
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                className="flex-grow-0 flex-shrink-0 w-6 h-6 relative"
                preserveAspectRatio="none"
              >
                <path
                  d="M11.5 9.05C12.417 8.113 13.611 7.5 15 7.5C16.4587 7.5 17.8576 8.07946 18.8891 9.11091C19.9205 10.1424 20.5 11.5413 20.5 13V20.5H18.5V13C18.5 12.0717 18.1313 11.1815 17.4749 10.5251C16.8185 9.86875 15.9283 9.5 15 9.5C14.0717 9.5 13.1815 9.86875 12.5251 10.5251C11.8687 11.1815 11.5 12.0717 11.5 13V20.5H9.5V8H11.5V9.05ZM4.5 6C4.10218 6 3.72064 5.84196 3.43934 5.56066C3.15804 5.27936 3 4.89782 3 4.5C3 4.10218 3.15804 3.72064 3.43934 3.43934C3.72064 3.15804 4.10218 3 4.5 3C4.89782 3 5.27936 3.15804 5.56066 3.43934C5.84196 3.72064 6 4.10218 6 4.5C6 4.89782 5.84196 5.27936 5.56066 5.56066C5.27936 5.84196 4.89782 6 4.5 6ZM3.5 8H5.5V20.5H3.5V8Z"
                  fill="white"
                />
              </svg>
            </div>
          </div>
    </div>
  </div>
</div>
    
  );
};

export default DetailPage;
